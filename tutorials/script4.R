...
dat %>%
  ggplot(aes(x = percollege, # add the independent variable
    ...
    geom_point() +
    labs(x = "Percentage of college educated adults",
         y = "Percentage of population below the poverty line",
        title = "Comparison of education and povert in the Midwest")
...
dat %>%
  cor.plot(filter(percblack, percbelowpoverty))
...
dat%>%
  ggplot(aes(x = percbelowpoverty, 
         ...
         data = ) +
    geom_point() +
    geom_smooth(method = "lm", formula = coll_pov)
